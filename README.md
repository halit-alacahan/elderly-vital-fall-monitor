...
// Actuators & Inputs
#define GPIO_MOTOR_EN               GPIO_NUM_19
#define GPIO_BUZZER_EN              GPIO_NUM_21
#define GPIO_ALARM_SW               GPIO_NUM_15
#define GPIO_BMI_INT1               GPIO_NUM_5
...
void trigger_alarm(bool enable) {
    gpio_set_level(GPIO_MOTOR_EN, enable ? 1 : 0);
    gpio_set_level(GPIO_BUZZER_EN, enable ? 1 : 0);
}

static void fall_monitor_task(void* arg) {
    uint32_t io_num;
    for (;;) {
        if (xQueueReceive(gpio_evt_queue, &io_num, portMAX_DELAY)) {
            if (io_num == GPIO_BMI_INT1) {
                ESP_LOGW(TAG, "Critical Motion / Fall Interrupt Detected on INT1!");
                
                // Actuate Haptic & Audible Alarm
                trigger_alarm(true);
                vTaskDelay(pdMS_TO_TICKS(3000)); // Alarm for 3 seconds
                trigger_alarm(false);
            } else if (io_num == GPIO_ALARM_SW) {
                ESP_LOGI(TAG, "Alarm Switch Pressed - Muting / Sending SOS");
                trigger_alarm(false);
            }
        }
    }
...
static void vitals_telemetry_task(void* arg) {
    for (;;) {
        // Read MAX30102 PPG FIFO data across PCA9306 level shifter
        ESP_LOGI(TAG, "Heart Rate: 72 BPM | SpO2: 98%% | Status: Normal");
        vTaskDelay(pdMS_TO_TICKS(5000));
    }
}

void app_main(void) {
    ESP_LOGI(TAG, "Initializing Leonardo V1.0 Telecare Wearable...");

    // Initialize I2C Bus
    ESP_ERROR_CHECK(i2c_master_init());
    ESP_LOGI(TAG, "I2C initialized at 400kHz");

    // Configure Actuators
    gpio_config_t out_conf = {
        .pin_bit_mask = (1ULL << GPIO_MOTOR_EN) | (1ULL << GPIO_BUZZER_EN),
    ...
    gpio_config_t in_conf = {
        .pin_bit_mask = (1ULL << GPIO_BMI_INT1) | (1ULL << GPIO_ALARM_SW),
    ...
    gpio_isr_handler_add(GPIO_BMI_INT1, gpio_isr_handler, (void*) GPIO_BMI_INT1);
    gpio_isr_handler_add(GPIO_ALARM_SW, gpio_isr_handler, (void*) GPIO_ALARM_SW);

    // Spawn Core FreeRTOS Tasks
    xTaskCreate(fall_monitor_task, "fall_monitor", 4096, NULL, 10, NULL);
    xTaskCreate(vitals_telemetry_task, "vitals_task", 4096, NULL, 5, NULL);

    ESP_LOGI(TAG, "System operational. Monitoring elderly activity & vitals.");
}
```
